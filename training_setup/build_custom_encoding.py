...
SEQUENTIAL codes 0..N-1 in that order (char_to_code/code_to_char) --
purely for deterministic reference/debugging, not the actual on-wire
encoding. Layered on top of that, char_to_bytes_hex/bytes_hex_to_char
assign each character a RANDOM (but seeded via --seed, so reproducible)
unique byte sequence of the decided width -- this is what makes the
final encoding look arbitrary rather than a straight renumbering of
Unicode codepoints, while still being exactly reproducible from the same
character set + seed.
...
Output (--out-path, default training_setup/custom_encoding.json):
  {
    ...
    "code_to_char": {"<int as string>": char, ...},
    "char_to_bytes_hex": {char: "<hex string>", ...},
    "bytes_hex_to_char": {"<hex string>": char, ...}
  }
  Hex strings are bytes_per_char*2 hex characters, e.g. "002a" for a
  2-byte code, "0a2bff" for a 3-byte code -- convert with
  bytes.fromhex(hex_key) / value.to_bytes(bytes_per_char, "big").hex().

Usage:
    python training_setup/build_custom_encoding.py
    python training_setup/build_custom_encoding.py --shard-roots data/lang_shards_balanced_4gpu data/lang_shards_imbalanced_4gpu --langs-csv training_setup/langs/langs_chosen.csv --out-path training_setup/custom_encoding.json --seed 42
...
def parse_args():
    ...
    parser.add_argument("--seed", type=int, default=42,
                         help="Seed for the random (but reproducible) byte-sequence "
                              "assignment -- re-running with the same seed against the "
                              "same character set always produces the identical mapping "
                              "(default 42).")
...
def main():
    ...
    # Random (but seeded -> reproducible) assignment of actual byte
    # sequences, layered ON TOP OF the deterministic index above rather
    # than replacing it: char_to_code/code_to_char stay a plain enumeration
    # (useful for reference/debugging), while char_to_bytes_hex is what
    # makes the actual on-wire encoding look arbitrary instead of just a
    # straight renumbering of Unicode codepoints. random.sample against a
    # range object is efficient even for a large capacity (doesn't
    # materialize the whole range), so this works fine up to the 3-byte
    # (16.7M) case too.
    rng = random.Random(args.seed)
    random_values = rng.sample(range(capacity), n_chars)  # unique per character

    char_to_bytes_hex = {}
    bytes_hex_to_char = {}
    for ch, value in zip(sorted_chars, random_values):
        hex_key = value.to_bytes(bytes_per_char, "big").hex()
        char_to_bytes_hex[ch] = hex_key
        bytes_hex_to_char[hex_key] = ch
    ...
    out = {
        ...
        "char_to_bytes_hex": char_to_bytes_hex,
        "bytes_hex_to_char": bytes_hex_to_char,
    }
